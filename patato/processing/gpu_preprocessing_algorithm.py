...
class GPUMSOTPreProcessor(TimeSeriesProcessingAlgorithm):
    ...
    def run(
        ...
        **kwargs,
        ...
        if pa_data.get_overall_correction_factor() is not None:
            overall_correction_factor = cp.array(
                pa_data.get_overall_correction_factor()
            )
        ...
        new_time_series, new_parameters = self._run(
            ...
            ft_filter,
            cp.array(detectors),
            cp.array(overall_correction_factor),
            **kwargs,
        )
    ...
    def interpolate(
        ...
        new_detector_ind = (
            cp.arange((detectors.shape[0] - 1) * self.detector_factor + 1)
            / self.detector_factor
        )
        if exact_ratios:
            new_detector_ind = cp.linspace(
        ...
        new_samp_ind = (
            cp.arange((signal.shape[-1] - 1) * self.time_factor + 1) / self.time_factor
        )
        if exact_ratios:
            new_samp_ind = cp.linspace(
                0, signal.shape[-1] - 1, self.time_factor * signal.shape[-1]
            )

        new_signal = cp.zeros(
            signal.shape[:-2] + (new_detector_ind.shape[0], new_samp_ind.shape[0]),
            dtype=signal.dtype,
        )
        t = time.time()
        ...
        new_signal += (
            signal[..., dind.astype(cp.int32), sind.astype(cp.int32)]
            * (1 - (dind - cp.floor(dind)))
            * (1 - (sind - cp.floor(sind)))
        )
        new_signal += (
            signal[..., dind.astype(cp.int32) + 1, sind.astype(cp.int32)]
            * (dind - cp.floor(dind))
            * (1 - (sind - cp.floor(sind)))
        )
        new_signal += (
            signal[..., dind.astype(cp.int32), sind.astype(cp.int32) + 1]
            * (1 - (dind - cp.floor(dind)))
            * (sind - cp.floor(sind))
        )
        new_signal += (
            signal[..., dind.astype(cp.int32) + 1, sind.astype(cp.int32) + 1]
            * (dind - cp.floor(dind))
            * (sind - cp.floor(sind))
        )
        #
        ...
        detectors = cp.apply_along_axis(
            lambda x: cp.interp(new_detector_ind, detector_ind, x), 0, detectors
        )
        ...
        new_data = PATimeSeries(
            new_signal, time_series.da.dims, coords, attributes=attributes
        )
