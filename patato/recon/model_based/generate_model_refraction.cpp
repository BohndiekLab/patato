...
double f(double x_0, double y_0, double c_0, double c_1, double x, double y) {
  // Derivative of time function
  return x_0 / (c_0 * sqrt(pow(x_0, 2) + pow(y_0, 2))) +
         (x_0 - x) / (c_1 * sqrt(pow(x_0 - x, 2) + pow(y_0 - y, 2)));
}

double fprime(double x_0, double y_0, double c_0, double c_1, double x,
              double y) {
  // Second derivative of time function
  return -x_0 * x_0 / (c_0 * pow(x_0 * x_0 + y_0 * y_0, 1.5)) +
         1 / (c_0 * sqrt(x_0 * x_0 + y_0 * y_0)) +
         1 / (c_1 * sqrt(pow(x_0 - x, 2) + pow(y_0 - y, 2))) -
         pow(x_0 - x, 2) / (c_1 * pow(pow(x_0 - x, 2) + pow(y_0 - y, 2), 1.5));
}

double get_refraction_point(double x, double y, double c0, double c1,
                            double y_cutoff) {
  double x_0 = x * (y_cutoff) / (y);
  double x_prev;
  double f_prev;
  double f_new = f(x_0, y_cutoff, c0, c1, x, y);
  int diverging = 0;
  for (int i = 0; i < 100; i++) {
    ...
    f_new = f(x_0, y_cutoff, c0, c1, x, y);
    if (f_new > f_prev) {
      // Diverging:
      diverging = 1;
      x_0 = x * (y_cutoff) / (y);
  ...
  //    if (diverging == 1){
  //        std::cout << x_0 << " " << x << " " << y << " " << c0 << " " << c1
  //        << " " << y_cutoff << std::endl;
  //    }
  return x_0;
}

double sector_integrate(double x, double y, double dx, double r, int qx,
                        int qy) {
  ...
  double terms[] = {dangle_1_dr,
                    ...
                    -dcdr * a / 2 * ca1 * ca1,
                    dcdr * a / 2 * ca0 * ca0,
                    a * c * ca1 * sa1 * dangle_1_dr,
                    -a * c * ca0 * sa0 * dangle_0_dr};
...
void calculate_element(py::array_t<double> output_py,
                       py::array_t<int> indices_py, int nx, int nt_pixel,
                       double detx, double dety, double dl_couple,
                       double dl_tissue, double x_0, double dx,
                       double y_couple) {
  ...
  for (int i = 0; i < nt_pixel * nx * nx; i++) {
    ...
    // So, we're calculating the weight for the pixel (x, y) and the time point
    // that is nt away from it's middle. Nothing needs changing above.
    ...
    if (y > y_couple) {
      ...
      double x_intercept =
          get_refraction_point(x, y, dl_couple, dl_tissue, y_cutoff);

      double N =
          sqrt(x_intercept * x_intercept + y_cutoff * y_cutoff) / dl_couple +
          sqrt((x_intercept - x) * (x_intercept - x) +
               (y_cutoff - y) * (y_cutoff - y)) /
              dl_tissue;
      double delta_x =
          (x - x_intercept) / sqrt((x - x_intercept) * (x - x_intercept) +
                                   (y - y_cutoff) * (y - y_cutoff));
      double delta_y =
          (y - y_cutoff) / sqrt((x - x_intercept) * (x - x_intercept) +
                                (y - y_cutoff) * (y - y_cutoff));
    ...
    // If this ever needs to be changed to float instead of doubles, be really
    // careful...
    weight = 0.;
    double v;
...
PYBIND11_MODULE(generate_model_refraction, m) {
