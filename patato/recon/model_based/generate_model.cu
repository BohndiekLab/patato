...
extern "C" __global__ void calculate_element(double *output, int *indices,
                                             ...
                                             double dety, double dl, double x_0,
                                             double dx) {
  int pixel;
  int nt_offset;
  int i_x;
  ...
  for (int i = tid; i < size; i += ntid) {
    ...
    // Probably overkill, but this does Kahan summation to try and minimise the
    // round off error. This part can probably be eliminated, it's more
    // important in the sector integrate function. I'm too scared to do this lol
    // ^. Thanks to this: if desired in future, this can *probably* be changed
    // to floats instead of doubles if speed/memory are a huge issue, but I
    // think it should be avoided if possible. Maybe you could convert the final
    // matrices to floats if you really are short of memory.
    double kahanc = 0.;
    weight = 0.;
    double kahans = 0.;
    double kahany;
