...
class FFTReconstruction(ReconstructionAlgorithm):
    ...
    def reconstruct(self, time_series: np.ndarray,
        ...
        time_series = time_series.reshape((int(np.prod(shape)), ) + time_series.shape[-2:])
        output = []
        self.hankels = None
        for i in range(time_series.shape[0]):
            output.append(self._reconstruct(time_series[i], fs, geometry, n_pixels,
                                            field_of_view, speed_of_sound, **kwargs))
        o = np.stack(output)
        return o.reshape(shape + o.shape[-2:])

    def _reconstruct(self, raw_timeseries_data: np.ndarray,
                    fs: float,
                    geometry: np.ndarray, n_pixels: Sequence[int],
                    field_of_view: Sequence[float],
                    speed_of_sound,
        ...
        n_samples_padded_grid = kwargs.get("n_samples_padded_grid", max(4096, raw_timeseries_data.shape[-1]))
        ...
        nonzero_axes = [i for i in range(geometry.shape[-1]) if not np.all(geometry[:, i] == 0)]
        
        if not np.all(np.isclose(detector_radii, detector_radius)):
            raise ValueError("All points on detector must be on a circle centred around (0, 0).")
        if not len(nonzero_axes) == 2:
            raise ValueError("Detectors must be 2-dimensional, i.e. geometry array must either be (ndet, 2) or (ndet, 3) with np.all(geometry[:, i] == 0) for a single value of i.")
        geometry = geometry[:, nonzero_axes]
        
        field_of_view = [f for i, f in enumerate(field_of_view) if n_pixels[i] not in [0, 1]]
        n_pixels = [f for f in n_pixels if f not in [0, 1]]
        
        if not len(field_of_view) == 2 or not len(n_pixels) == 2 or field_of_view[1] != field_of_view[0] or n_pixels[0] != n_pixels[1]:
            raise ValueError()
        ...
        detector_angles = (np.arctan2(geometry[:, 1], geometry[:, 0]) - mean_angle) % (2 * np.pi)
        detector_angles[detector_angles > np.pi] = detector_angles[detector_angles > np.pi] - np.pi * 2
        ...
        if debug:
            ...
            plt.plot(raw_timeseries_data[128] * hatfunction, 
                    label="Smooth timeseries cut")
            plt.plot(hatfunction * np.max(raw_timeseries_data[128]), c="C2", 
                    label="Hat function")
        ...
        new_angles_detectors = np.linspace(-np.pi, np.pi, n_grid_detectors, endpoint=False)
        ...
        timeseries[:, :timeseries_data.shape[1]] = np.stack([np.interp(new_angles_detectors,
                                                                       detector_angles,
                                                                       timeseries_data[:, i], 
                                                                       0, 0
                                                                       ) for i in range(nt)]).T
        ...
        # 3. Fourier Transform
        nfft_positive = n_samples_padded_grid // 2 if n_samples_padded_grid % 2 == 1 else n_samples_padded_grid // 2 - 1
        ...
        if self.hankels is None:
            ...
            indices, frequencies = np.meshgrid(indices_detector_grid, freq_rad[1:], indexing="ij")
            ...
            hankels[1:n_grid_detectors // 2, 1:] = hankel_temp[::-1][:n_grid_detectors // 2-1]
        ...
        csum = np.sum(ft_timeseries[zero_frequency_index, 1:] * j1[1:] / freq_rad[1:]) * (freq_rad[1] - freq_rad[0])
        ...
        if debug:
            ...
            plt.show()
        interp_spline_r = RectBivariateSpline(np.linspace(0, 2 * np.pi, n_grid_detectors + 1),
                                              positive_time_frequencies, tran_ext.real
                                              )
        interp_spline_i = RectBivariateSpline(np.linspace(0, 2 * np.pi, n_grid_detectors + 1),
                                              positive_time_frequencies, tran_ext.imag
                                              )
        
        cart_transform = interp_spline_r(freq_theta, freq_rho, grid=False) + \
        ...
        if(lhalf == 1):
            cart_transform[:mcenter,:] = np.conj(np.copy(np.flip(cart_transform[-mcenter:,:],[0,1])))
        if(lhalf == 2):
            cart_transform[-mcenter:,:] = np.conj(np.copy(np.flip(cart_transform[:mcenter,:],[0,1])))
        if(lhalf == 3):
            cart_transform[:,:mcenter] = np.conj(np.copy(np.flip(cart_transform[:,-mcenter:],[0,1])))
