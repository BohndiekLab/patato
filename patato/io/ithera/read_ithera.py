...
from ..ithera.iannotation import IAnnotation
...
class iTheraMSOT(ReaderInterface):
    ...
    def _get_rois(self):
        """
        load annotations generated by ithera ilabs software, which is stored in the *.iannotation file.
        """
        output = {}

        iannotation_file = join(
            self.scan_folder, split(self.scan_folder)[-1] + ".iannotation"
        )

        if not exists(iannotation_file):
            return output
