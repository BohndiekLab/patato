...
def read_reconstruction_preset(json_path: Union[str, dict]):
    ...
    time_interpolation_factor = settings.get(
        PreprocessingAttributeTags.TIME_INTERPOLATION, 3
    )
    detector_interpolation_factor = settings.get(
        PreprocessingAttributeTags.DETECTOR_INTERPOLATION, 2
    )

    correct_for_impulse_response = settings.get(
        PreprocessingAttributeTags.IMPULSE_RESPONSE, True
    )
    do_hilbert_transform = settings.get(
        PreprocessingAttributeTags.HILBERT_TRANSFORM, False
    )
    do_envelope_detection = settings.get(
        PreprocessingAttributeTags.ENVELOPE_DETECTION, False
    )
    ...
    preprocessing_algorithm = PREPROCESSING_NAMES[
        settings[PreprocessingAttributeTags.PROCESSING_ALGORITHM]
    ]

    # Reconstruction parameters.
    recon_params = settings.get(
        ReconAttributeTags.ADDITIONAL_PARAMETERS, {}
    )  # Custom parameters

    # Field of view y and z:
    field_of_view_x = settings.get(ReconAttributeTags.X_FIELD_OF_VIEW, 0.0)
    field_of_view_y = settings.get(ReconAttributeTags.Y_FIELD_OF_VIEW, 0.0)
    field_of_view_z = settings.get(ReconAttributeTags.Z_FIELD_OF_VIEW, 0.0)
    ...
    step_1 = preprocessing_algorithm(
        time_factor=time_interpolation_factor,
...
def read_unmixing_preset(
    json_path: Union[str, dict], example_data: Optional[PAData], **kwargs
):
    """
    ...
    pipeline = SpectralUnmixer(
        spectra, wavelengths, resolution_reduction_factor, algorithm_id=suffix
    )

    # If haemoglobin and oxyhaemoglobin are present in the spectra, add THb and SO2 calculations to the pipeline
    if (
        UnmixingAttributeTags.HAEMOGLOBIN in spectra_names
        and UnmixingAttributeTags.OXYHAEMOGLOBIN in spectra_names
    ):
