...
class TestITheraImport(unittest.TestCase):
    def test_overall_processing(self):
        ...
        self.assertEqual(
            pa_1.scan_reader.get_scan_comment(), pa_2.scan_reader.get_scan_comment()
        )
        ...
        preproc = DefaultMSOTPreProcessor(
            time_factor=time_factor, detector_factor=detector_factor
        )
        recon = ReferenceBackprojection(
            field_of_view=(0.025, 0.025, 0.025), n_pixels=(333, 333, 1)
        )
        ...
        rois_2 = [roi.points for roi in pa_2.get_rois().values()]
        self.assertTrue(
            all(np.allclose(roi1, roi2) for roi1, roi2 in zip(rois_1, rois_2))
        )
