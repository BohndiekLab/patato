...
class TestMSOTData(unittest.TestCase):
    def test_msot_data(self):
        ...
        self.assertEqual(
            pa.get_scan_reconstructions().raw_data.shape, (1, 2, 1, 333, 333)
        )
        ...
        self.assertTrue(
            np.all(
                np.squeeze(pa.get_scan_so2_time_mean().raw_data)
                == np.squeeze(np.mean(pa.get_scan_so2().raw_data, axis=0))
            )
        )
        self.assertTrue(
            np.all(
                np.squeeze(pa.get_scan_thb_time_mean().raw_data)
                == np.squeeze(np.mean(pa.get_scan_thb().raw_data, axis=0))
            )
        )
        pa.close()
